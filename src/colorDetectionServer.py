...
def loadValues():
    ...
    try:
        arq = open('/home/vinicius/catkin_ws/src/calibrationColors.txt', 'r')
        text = arq.readlines()
        ...
        for line in text:
            ...
            if content[0] == 'BLUE_MIN':
                ...
                blue_min = np.array([float(content[1]), float(content[2]), float(content[3])])
                print(blue_min)

            elif content[0] == 'BLUE_MAX':
                # for i in range (1,len(content)):
                #     blue_max.append(float(content[i]))
                blue_max = np.array([float(content[1]), float(content[2]), float(content[3])])

            elif content[0] == 'GREEN_MIN':
                # for i in range (1,len(content)):
                #     green_min.append(float(content[i]))
                green_min = np.array([float(content[1]), float(content[2]), float(content[3])])

            elif content[0] == 'GREEN_MAX':
                # for i in range (1,len(content)):
                #     green_max.append(float(content[i]))
                green_max = np.array([float(content[1]), float(content[2]), float(content[3])])

            elif content[0] == 'RED_MIN':
                # for i in range (1,len(content)):
                #     red_min.append(float(content[i]))
                red_min = np.array([float(content[1]), float(content[2]), float(content[3])])

            elif content[0] == 'RED_MAX':
                # for i in range (1,len(content)):
                #     red_max.append(float(content[i]))
                red_max = np.array([float(content[1]), float(content[2]), float(content[3])])
...
def handle_detect_color(req):
    ...
    if verificador == "azul" and len(contornosBlue) > 0:
        ...
        cv2.drawContours(frame, maior_contorno, -1, (255,0,0), 3)
        ...
        if raio > 10:
            ...
            print ("foi pro verde")
    if verificador == "verde" and len(contornosGreen) > 0:
        print("Blue containner couldn't be founded")
        maior_contorno = max(contornosGreen, key=cv2.contourArea)
    ...
    pub = rospy.Publisher('centroidePosition', std_msgs.msg.String, queue_size=10)
    pub.publish(std_msgs.msg.String(str(centroide[0])))
